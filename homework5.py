...
#immutable_var[2] = 'камера' # 'tuple' object does not support item assignment
                             ...
                             # внутри кортежа. Каждый объект в кортеже привязывается к определенному
                             # номеру, этот номер являет собой значение привязанного объекта.
#immutable_var.remove(15.0)
#immutable_var[5][0] = immutable_var[6][0]  # (14, 15.0, 'микрофон', [2], [2], ['камера', 'камера'], ['камера', 'микрофон'])
                                            # Мы можем изменять данные внутри списков, что находятся в кортеже,
                                            # поскольку они являются объектами со своим внутренним набором данных.

#immutable_var[5], immutable_var[6] = immutable_var[6], immutable_var[5]  # 'tuple' object does not support item assignment
                                                                          # Однако мы так не можем менять местами списки внутри
                                                                          # кортежа, поскольку каждый список в кортеже так же является
                                                                          # объектом cо своим номером, а значит не изменяется самим кортежем.
