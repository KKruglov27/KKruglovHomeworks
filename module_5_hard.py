...
class UrTube():
    ...
    def add_video(self, *args):
        for elem1 in args:
            ...
            self.videos_name.append(elem1.tittle)
            ...
            time.sleep(1)
        print()
        input(f"{Fore.LIGHTWHITE_EX}Вывести список видео?{Style.RESET_ALL} 'Дa' 'Нет', Введите ниже пожалуйста :)")
        if input() == "Да":
            print(f'{Fore.BLUE}Загрузка...{Style.RESET_ALL}')
            time.sleep(1.5)
            print(f'{Fore.LIGHTWHITE_EX}Список видео:{Style.RESET_ALL}')
            time.sleep(1)
            num1 = 1
            for video in self.videos_name:
                print('#', num1, video)
                time.sleep(0.1)
                num1 += 1
    ...
    def get_videos(self, video_search):
        ...
        for videos_list in self.videos:
            if isinstance(videos_list, list):
                ...
                for word in search:
                    if word in word_list and videos_list[0] not in self.found_videos:
    ...
    def watch_video(self, play_video):
        ...
        print(f'Попытка воспроизведения видео {Fore.LIGHTWHITE_EX}"{play_video}"{Style.RESET_ALL}:')
        time.sleep(1)
        if play_video not in self.videos_name:
            print(f"{Fore.RED}ERROR: __this video is not defined__{Style.RESET_ALL}")
            return
               ...
               if (i[0]) == self.current_user:
                for chek in self.videos:
                    if chek[0] != self.play_video:
                        ...
                        if chek[3] is True and i[1] >= 18 or chek[3] is False:
                            duration = chek[1]
...
v3 = Video('Как перейти в "Customize"?', 12)
